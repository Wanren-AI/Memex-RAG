...
> **智能文档问答系统** | 企业级 RAG 解决方案 | 支持全文档检索与智能过滤

一个专业的检索增强生成（RAG）系统，专为企业级文档分析场景设计，支持多文档智能检索、LLM相关性评估、灵活的文档管理以及完整的对话历史记忆。
...
#### 1. **智能两阶段检索（LLM增强）**
传统RAG系统仅依赖向量相似度，容易检索到语义相似但实际不相关的内容。本系统创新性地引入**两阶段检索架构**：
...
```
查询："巴菲特2023年的投资策略是什么？"
传统RAG：可能召回包含"2023"和"策略"但谈论其他主题的内容
智能模式：LLM评估后仅保留真正讨论巴菲特2023年投资策略的chunks
```

#### 2. **灵活的文档管理系统**
解决上一版本RAG系统"一次性构建，难以修改"的痛点，特别适合需要频繁更新文档的场景（如银行规章、法律条文）：
...
**本系统的解决方案**：
- ✅ **独立索引**：每个文档单独存储向量索引，互不干扰
- ✅ **智能更新检测**：基于MD5哈希自动检测文档变化
- ✅ **热更新支持**：修改文档仅重建该文档的索引，秒级完成
- ✅ **延迟删除机制**：解决Windows文件锁问题，优雅处理资源清理
